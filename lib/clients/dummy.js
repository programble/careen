...
/**
 * Interface of SQL database clients.
 * @interface Client
 */
 ...
 * @param {Client~Connection} db - Database connection
 * @param {string} tableName - Name of the journal table
 * @param {module:runner.OPERATION} operation - Migration operation
 ...
 * @property {module:runner.OPERATION} operation - Migration operation
...
function runMigrationSQL(db, sql) {
  if (sql.indexOf('ERROR') === 0) return Promise.reject(new Error(sql));
