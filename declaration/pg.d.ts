...
declare module 'pg' {
  ...
  export class Query extends events.EventEmitter {
    on(event: 'row', listener: (row: any, result?: ResultBuilder) => void): Query;
    on(event: 'error', listener: (err: Error) => void): Query;
    on(event: 'end', listener: (result: ResultBuilder) => void): Query;
    on(event: string, listener: Function): Query;
  }

  export class Events extends events.EventEmitter {
    on(event: 'error', listener: (err: Error, client: Client) => void): Events;
    on(event: string, listener: Function): Events;
  }
}
