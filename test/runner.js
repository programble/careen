  ...
  describe('applyEach', function() {
    describe('without error', function() {
      ...
      it('calls appendJournal for each', function() {
        assert.equal(Dummy.appendJournal.callCount, 2);
      });
      it('calls commitTransaction for each', function() {
        assert.equal(Dummy.commitTransaction.callCount, 2);
      });
      it('calls disconnect', function() {
        assert.equal(Dummy.disconnect.callCount, 1);
      });
    ...
    describe('with error', function() {
      ...
      it('calls runMigrationSQL', function() {
        assert.equal(Dummy.runMigrationSQL.callCount, 1);
      });
      it('does not call appendJournal', function() {
        assert.equal(Dummy.appendJournal.callCount, 0);
      });
      ...
      it('rethrows runMigrationSQL', function() {
        Promise.join(
          ...
          Dummy.runMigrationSQL.firstCall.returnValue.then(null, R.identity),
          assert.equal
        );
      });
    });
